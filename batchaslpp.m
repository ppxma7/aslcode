...
pathname ='/Volumes/ares/data/IBD/ASL/ASL_jord/';
...
subjectlist={'003','004','005','006','008','011','012',...
    '014','020','021','022','024','025','026','027',...
    '028','031','032','033','034','038'};
%
cd(pathname)
...
for ii = 1:length(subjectlist)
    ...
    for jj=1:length(allFileNames)
        aslpp(allFileNames{jj},allFileNames2{1}); %JORD HAS ONE BASE ONLY
...
%%
% pathname2 ='/Volumes/ares/data/IBD/ASL/ASL_gita/Patients/';
% 
...
% cd(pathname2)
% %subjectlist = {'001_H07_V2'};
% for ii = 1:length(subjectlistP)
%     cd([pathname2 subjectlistP{ii}])
%     % build a name
%     % Get a list of all files in the folder with the desired file name pattern.
%     myFolder = pwd; % Wherever you want.
%     filePattern = fullfile(myFolder, '*label1*'); % Change to whatever pattern you need, for example *.m or whatever...
